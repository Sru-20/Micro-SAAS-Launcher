...
type AuthContextValue = {
  user: AuthUser | null;
  isReady: boolean;
  /** Demo-only: set a local profile when Supabase env is missing. */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signInWithGoogle = useCallback(async (nextPath = "/manage/listings") => {
...
export function LoginCard() {
  ...
  const next = params.get("next") || "/manage/listings";
  ...
  return (
    <div className="auth-card">
      <div className="auth-card-head">
        ...
        <p className="auth-subtitle">Sign in to manage listings.</p>
      </div>
      ...
      {err === "user_sync_failed" && (
        <p className="muted small" style={{ color: "#fca5a5" }}>
          ...
          RLS allows inserts for <code>auth.uid()</code> (see <code>supabase-rls-users.sql</code> in
          this project).
        </p>
      )}
        ...
        {hasSupabaseEnv() ? (
          <>
            ...
            <p className="muted small">
              Uses your Supabase project: sessions are stored in cookies so listings sync to your
              database under the same account that owns this site&apos;s project.
            </p>
          </>
        ) : (
          <>
            ...
            >
              Sign in (demo) <span className="btn-arrow">→</span>
            </button>
            <p className="muted small">
