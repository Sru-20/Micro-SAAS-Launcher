...
function logSyncError(context: string, message: string) {
  if (process.env.NODE_ENV === "development") {
    console.warn(`[listings sync] ${context}:`, message);
  }
}
...
async function syncListingsToSupabase(rows: Listing[]) {
  ...
  const { error } = await supabase.from(dbTableName("listings")).upsert(
    rows.map(normalizeListingForDb),
    { onConflict: "id" }
  );
  if (error) {
    logSyncError("upsert", error.message);
  }
}
...
export function deleteListing(id: string) {
  ...
  void supabase
    ...
    .eq("id", id)
    .then(({ error }) => {
      if (error) logSyncError("delete", error.message);
    });
}
